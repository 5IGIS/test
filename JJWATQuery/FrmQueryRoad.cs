...
{
    public partial class FrmQueryRoad : Form
    {
        public FrmQueryRoad()
        {
            InitializeComponent();
        ...
        private void frmQueryRoad_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (m_lsLayer.Count > 0)
                {
                    foreach (var layer in m_lsLayer)
                    {
                        ...
                        foreach (string value in modMain.m_CurParrmeter.QueryAddr())
                        {
